...
class NPModel(Basemodel):
    ...
    def preprocess(self, image):
        input_shape = self.inputs[next(iter(self.inputs))].tensor_desc.dims
        layout = self.inputs[next(iter(self.inputs))].tensor_desc.layout
        input_size = list(map(int, input_shape))[2:]
